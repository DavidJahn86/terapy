...
#            noise=noise[:,1]
    ...
    def calcunc(self,tdDatas):
        ...
        if tdDatas.shape[0]==1:
            repeatability=py.zeros((len(tdDatas[0,:,0]),))
        else:
            repeatability=py.std(py.asarray(tdDatas[:,:,1]),axis=0, ddof = 1)/py.sqrt(self.numberOfDataSets)

        precNoise=self.getPreceedingNoise()
        elnNoise = py.std(precNoise, ddof = 1)*py.ones(len(repeatability))/py.sqrt(len(precNoise))
